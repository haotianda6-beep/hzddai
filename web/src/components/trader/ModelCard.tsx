...
export function ModelCard({ model, selected, onClick, configured }: ModelCardProps) {
  return (
    <button
      ...
      style={{
        background: selected ? 'rgba(212, 255, 51, 0.12)' : '#0b0b0b',
        border: selected ? '2px solid #c4cf45' : '2px solid #2B3139',
      }}
    >
      <div className="relative">
        <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-black border border-white/10">
          {getModelIcon(model.provider || model.id, { width: 32, height: 32 }) || (
            <span className="text-lg font-bold" style={{ color: '#dce76a' }}>{model.name[0]}</span>
          )}
        </div>
        {selected && (
          <div
      ...
      <span
        ...
        style={{ background: 'rgba(212, 255, 51, 0.2)', color: '#dce76a' }}
      >
        {model.provider}
      </span>
    </button>
